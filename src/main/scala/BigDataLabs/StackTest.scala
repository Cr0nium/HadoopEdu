...
object StackTest {
  def main(args: Array[String]) {
    ...
    p.addColumn(Bytes.toBytes("0"), Bytes.toBytes("NAME"),Bytes.toBytes("raju"))
    p.addColumn(Bytes.toBytes("0"), Bytes.toBytes("COURSE"),Bytes.toBytes("SCALA"))
    p.addColumn(Bytes.toBytes("0"), Bytes.toBytes("YEAR"),Bytes.toBytes("2017"))
    table.put(p)
